...
-- Drop storage policies that reference the role column
DROP POLICY IF EXISTS "Teachers can view assignment files" ON storage.objects;
DROP POLICY IF EXISTS "Teachers can download assignment files" ON storage.objects;
DROP POLICY IF EXISTS "Teachers can view all assignment files" ON storage.objects;

-- Now change the role column type to text
...
-- Recreate the assignment policies
