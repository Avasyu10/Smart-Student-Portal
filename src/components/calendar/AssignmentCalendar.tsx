...
const AssignmentCalendar: React.FC<AssignmentCalendarProps> = ({ className }) => {
  ...
  const getAssignmentStatus = (assignment: Assignment) => {
    ...
    if (submission) {
      if (submission.grade !== null && submission.grade !== undefined) {
        return { status: 'graded', label: 'Graded', color: 'bg-green-500' };
      }
      return { status: 'submitted', label: 'Submitted', color: 'bg-blue-500' };
    }
    ...
    if (daysUntilDue <= 2) {
      return { status: 'urgent', label: 'Due Soon', color: 'bg-orange-500' };
    }
  ...
  const modifiers = {
    hasAssignment: (date: Date) => getAssignmentForDate(date).length > 0,
    ...
    submitted: (date: Date) => {
      const dayAssignments = getAssignmentForDate(date);
      return dayAssignments.every(assignment => {
        const submission = getSubmissionStatus(assignment.id);
        return !!submission;
      }) && dayAssignments.length > 0 && getPersonalEventsForDate(date).length === 0;
    }
  };
      ...
      <Card className={className}>
        ...
        <CardContent>
          <div className="space-y-6">
            ...
            {/* Properly Sized Calendar Layout */}
                  ...
                  <div className="space-y-4">
                    ...
                    {selectedAssignments.length > 0 && (
                      <div className="space-y-3">
                        <h5 className="text-sm font-medium text-muted-foreground">ASSIGNMENTS</h5>
                          ...
                          return (
                            <div key={assignment.id} className="p-3 border rounded-lg space-y-2">
                              <div className="flex items-start justify-between">
                                ...
                                <Badge variant={status.status === 'graded' ? 'default' : 'outline'} className="text-xs">
                                  {status.label}
                                </Badge>
                              </div>
                      ...
                      <div className="space-y-2 max-h-48 overflow-y-auto">
                        {assignments
                          ...
                          .map(assignment => {
                            ...
                            return (
                              <div key={assignment.id} className="flex items-center justify-between p-2 rounded border hover:bg-accent/50 cursor-pointer text-sm"
